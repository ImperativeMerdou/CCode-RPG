# FEATS — the trophy wall

Deeds with names. Each earned feat is permanent: it pays a rider (a lasting edge, written plainly) and fame (bounty or reputation, per the arithmetic). Locked feats are visible so the hunger has targets. The ??? rows are real, already defined in the vault, and land like thunder when hit.

| Feat | How | Rider | Status |
|---|---|---|---|
| First Blood | Drop your first named threat | +10 bounty; the street learns your face | LOCKED |
| The Fifth Head | Go the distance with the Anvil, win or lose | Respect in every pit on the coast | LOCKED |
| Ink | Appear in a broadsheet for the first time | A public exists that believes in you | LOCKED |
| One Warning | Defy the Guild openly and live a tenday | Independents start watching you | LOCKED |
| Head Money | Redeem your first mark at a gate | Hunters' guild rates you; marks come easier | LOCKED |
| The Long Road | Survive your first journey beyond the walls | One region's table holds no surprises for you again | LOCKED |
| Named | Earn an epithet on screen (never self-given) | The epithet does work: fear arrives before you do | LOCKED |
| Fifty Suns | Take the Midsummer tournament purse | The purse, and every pit boss on the coast knows | LOCKED |
| Papers | Free someone from an indenture | A loyalty no coin buys | LOCKED |
| The Reader | Learn what the mark on your back says | (the rider is the knowing) | LOCKED |
| First Rung | Settle the score with Keth the Brick | The pit rung clears; the street rung faces you | LOCKED |
| Crew | Three sworn names under you | The crew file opens for real | LOCKED |
| ??? | (vault) | (vault) | SEALED |
| ??? | (vault) | (vault) | SEALED |
| ??? | (vault) | (vault) | SEALED |
